...
<div>
    ...
    <p><?php _e('The external workaround does enable many features which are not possible directly because of cross domain security restrictions. You need to include a Javascript file (ai_exernal.js) to the page in the iframe which is generated dynamically with your settings. Please follow the instructions below. In the advanced tab are already many settings marked with ', 'advanced-iframe');
    echo renderExternalWorkaroundIcon(true);
    _e('. This means that this setting is saved to the ai_exernal.js', 'advanced-iframe') ?>
    </p>
    <p><b><?php _e('You need to be able to modify the external web page you want to have in the iframe to use the workaround!', 'advanced-iframe') ?></b></p>
...
if (empty($devOptions['document_domain'])) {
  ...
  $devOptions['document_domain'] = $currentDomain;
         ...
         printTextInput(false,$devOptions, __('Main domain', 'advanced-iframe'), 'document_domain', __('blub blub blub', 'advanced-iframe'), 'text', '',false);
?>
</table>
<p>
